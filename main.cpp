...
int main () {
    std::string path;
    std::cout<<"Enter the directory for the logfile (/home/user/example/" <<std::endl;
    std::cin >> path;
//    std::string path = "/home/stavrow/Logger/build/";
    ...
    while (true) {
        ...
        if (valid_input){
            Log.logInput(type, value); // Inputs: type (std::string) and value (void*)
        }
        else{
            std::cout<<"Invalid type given"<<std::endl;
        }
    }
    return 0;
}
